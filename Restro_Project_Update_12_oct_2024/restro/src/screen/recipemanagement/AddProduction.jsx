...
const headers = ["SL.", "Food Name", "Item Quantity", "Variant Name","Production date ", "Expiry date", ];
const ActionButtons = [
...
const [formData, setFormData] = useState({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/productiondata`, formData,{headers:{"Authorization":token}});
  
      // Check for the error message from the backend
      ...
      // If no error, proceed to show success message
  ...
  item.food_name
    ? `${item.food_name}`
    : "No food Found",
    Variant_Name: item.variant_name?`${item.variant_name}`:"No varient found",
...
new Date(item.saveddate).toLocaleDateString(),
 ...
 "Food Name", "Item Quantity", "Variant Name","Production date ", "Expiry date"
      ...
      <div className="main_div ">
        <section className=" side_section flex">
          ...
          <div className=" contant_div w-full  ml-4 pr-7 mt-4 ">
            ...
            >
              <div className="flex justify-center gap-x-3 items-center col-span-1">
                <label
                  className="block text-nowrap font-semibold mb-2"
                  htmlFor="foodName"
                >
                  Food Name*
                </label>
                      ...
                      {productionData.length > 0 ? (
                        productionData
                          ...
                          <tr key={index}>
                            ...
                            <td className="py-2 px-4 border border-[#4CBBA1]">
                              {row.itemquantity ? row.itemquantity:"Not found"}
                            </td>
                            <td className="py-2 px-4 border border-[#4CBBA1]">
                              {row.variant_name ? row.variant_name:"Not found"}
                            </td>
                            <td className="py-2 px-4 border border-[#4CBBA1]">
                              {row.saveddate ? new Date( row.saveddate).toLocaleDateString():"Not found"}
                            </td>
                            <td className="py-2 px-4 border border-[#4CBBA1]">
                              {row.productionexpiredate ? new Date( row.productionexpiredate).toLocaleDateString():"Not found"}
                            </td>
                            {/* <td className="py-2 px-4 border border-[#4CBBA1]">
