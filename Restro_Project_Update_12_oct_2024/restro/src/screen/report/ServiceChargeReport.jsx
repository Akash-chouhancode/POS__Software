...
const ServiceChargeReport = () => {
    ...
    const totalAmount= serviceData.reduce((acc,row)=> acc + parseFloat(row.service_charge || 0),0);
    ...
    const getServiceSaledata = () => {
        ...
        axios
          .get(`${API_BASE_URL}/servicechargereport`)
          .then((response) => {
...
const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/servicechargereport`, {
        params: {
          from: fromDate,
          to: toDate,
        },
                        ...
                        serviceData
                          ...
                          .map((row, index) => (
                            <tr key={index} className="border-b text-center ">
                              ...
                              <td className="py-2 px-4 border border-[#4CBBA1]">
                                {row.order_id? row.order_id :"No Id"}
                              </td>

                              <td className="py-2 px-4 border border-[#4CBBA1]">
                                {(row.service_charge).toFixed(2) ? (row.service_charge).toFixed(2) :"Not defined" }
                              </td>
                ...
                {serviceData.length > 0 && (
                  <div className="mt-10">
                    <div className="float-right flex items-center space-x-2">
                      ...
                      {[...Array(Math.ceil(serviceData.length / itemsPerPage))].map(
                        (_, index) => {
                          return (
                            <button
                      ...
                      <button
                        onClick={() => selectPage(currentPage + 1)}
                        disabled={
                          currentPage === Math.ceil(serviceData.length / itemsPerPage)
                        }
                        className="h-[46px] w-[70px] cursor-pointer border-[1px] border-[#1C1D3E] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
