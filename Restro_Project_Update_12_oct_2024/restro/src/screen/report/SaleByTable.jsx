...
const SaleByTable = () => {
  ...
  const [tablesaleData, setTableSaleData] = useState([]);

  const { isFullScreen, toggleFullScreen } = useFullScreen();
  const selectPage = (page) => {
      if (page > 0 && page <= Math.ceil(tablesaleData.length / itemsPerPage)) {
        setCurrentPage(page);
      }
    };
...
const totalAmount= tablesaleData.reduce((acc,row)=> acc + parseFloat(row.total_amount || 0),0);
  ...
  const gettableSaledata = () => {
  
      axios
        .get(`${API_BASE_URL}/tablebysale`)
        .then((response) => {
          console.log("datta",response)
          setTableSaleData(response.data.data);
        })
        .catch((error) => {
          console.error(error);
        });
...
const handleSearch = async (e) => {
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/tablebysale`, {
      params: {
        from: fromDate,
        to: toDate,
      },
    });
    setTableSaleData(response.data.data); // Update the displayed data
                        ...
                        tablesaleData
                          ...
                          .map((row, index) => (
                            <tr key={index} className="border-b text-center ">
                              ...
                              <td className="py-2 px-4 border border-[#4CBBA1]">
                                {row.tablename? row.tablename:"No Name found"}
                              </td>

                              <td className="py-2 px-4 border border-[#4CBBA1]">
                                {(row.total_amount) ? parseFloat(row.total_amount).toFixed(2) :"Not defined" }
                              </td>
                ...
                {tablesaleData.length > 0 && (
                  <div className="mt-10">
                    <div className="float-right flex items-center space-x-2">
                      ...
                      {[...Array(Math.ceil(tablesaleData.length / itemsPerPage))].map(
                        (_, index) => {
                          return (
                            <button
                      ...
                      <button
                        onClick={() => selectPage(currentPage + 1)}
                        disabled={
                          currentPage === Math.ceil(tablesaleData.length / itemsPerPage)
                        }
                        className="h-[46px] w-[70px] cursor-pointer border-[1px] border-[#1C1D3E] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
