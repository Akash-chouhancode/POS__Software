...
const headers = ["SL.", "Waiter Name", "Total Amount"];

const WaiterSaleReport = () => {
  ...
  const totalAmount = waiterData.reduce(
  ...
  const getWaiterSaledata = () => {
    axios
      .get(`${API_BASE_URL}/watersalereport`)
      .then((response) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/watersalereport`, {
        params: {
          from: fromDate,
          to: toDate,
        },
  ...
  const downloadExcel = async () => {
    ...
    worksheet.columns = [
      { header: "Waiter Name", key: "WaiterName" },
                ...
                {waiterData.length > 0 && (
                  <div className="mt-10">
                    <div className="float-right flex items-center space-x-2">
                      ...
                      {[
                        ...Array(Math.ceil(waiterData.length / itemsPerPage)),
                      ].map((_, index) => {
                        return (
                          <button
                      ...
                      <button
                        ...
                        disabled={
                          currentPage ===
                          Math.ceil(waiterData.length / itemsPerPage)
                        }
                        className="h-[46px] w-[70px] cursor-pointer border-[1px] border-[#1C1D3E] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
...
export default WaiterSaleReport;
