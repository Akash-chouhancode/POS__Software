...
const headers = ["SL.", "Cashier Name", "Total Amount"];

const SaleReportCashier = () => {
  ...
  const selectPage = (page) => {
    if (page > 0 && page <= Math.ceil(cashData.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  ...
  const getCashSaledata = () => {
    axios
      .get(`${API_BASE_URL}/casherreport`)
      .then((response) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/casherreport`, {
        params: {
          from: fromDate,
          to: toDate,
        },
      });
      setCashData(response.data.data); // Update the displayed data
  ...
  const downloadExcel = async () => {
    ...
    worksheet.columns = [
      { header: "Cashier Name", key: "cashier_name" },
  ...
  const downloadPDF = async () => {
    ...
    const rows = data.map((item) => [
      item.CashierName ? `${item.CashierName}` : "No Cashier Found",
      item.totalamount,
    ]);
    ...
    // Add a table
    doc.autoTable({
      head: [["Cashier Name", "Total Amount"]],
      body: rows,
    });
                ...
                {cashData.length > 0 && (
                  <div className="mt-10">
                    <div className="float-right flex items-center space-x-2">
                      ...
                      {[
                        ...Array(Math.ceil(cashData.length / itemsPerPage)),
                      ].map((_, index) => {
                        return (
                          <button
                      ...
                      <button
                        ...
                        disabled={
                          currentPage ===
                          Math.ceil(cashData.length / itemsPerPage)
                        }
                        className="h-[46px] w-[70px] cursor-pointer border-[1px] border-[#1C1D3E] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
...
export default SaleReportCashier;
