...
const OnlineOrder = () => {
  ...
  const [onlindData, setOnlineData] = useState([]);
  ...
  const getOnlineData = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/onlineOrders`);
  ...
  const handleSearch = (e) => {
    ...
    axios
      .get(`${API_BASE_URL}/onlineOrders`, {
        params: { searchItem: value },
      })
      .then((res) => {
        setOnlineData(res.data.data.length > 0 ? res.data.data : []);
      })
      .catch((error) => {
  ...
  const selectPage = (page) => {
    if (page > 0 && page <= Math.ceil(onlindData.length / itemsPerPage)) {
      setCurrentPage(page);
    }
  };
  ...
  const DeletModal = ({ isOpen, onClose, order_id }) => {
    ...
    const cancelOrder = (order_id) => {
      axios
        ...
        .then((response) => {
          console.log(response.data);
          getOnlineData(); // Assuming this fetches updated order data
                ...
                {onlindData.length > 0 && (
                  <div className="mt-10">
                    <div className="float-right flex items-center space-x-2">
                      ...
                      {[
                        ...Array(Math.ceil(onlindData.length / itemsPerPage)),
                      ].map((_, index) => {
                        return (
                          <button
                      ...
                      <button
                        ...
                        disabled={
                          currentPage ===
                          Math.ceil(onlindData.length / itemsPerPage)
                        }
                        className="h-[46px] w-[70px] cursor-pointer border-[1px] border-[#1C1D3E] disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next
